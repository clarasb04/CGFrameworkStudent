...
void Entity::Render(Image* framebuffer, Camera* camera, const Color& c, bool zBuf_flag, FloatImage* zBuf, bool textur) {
	...
	for (int i = 0; i < malla.GetVertices().size(); i+=3) {
		...
		case eRenderMode::TRIANGLES_INTERPOLATED:
			if (textur) {
				framebuffer->DrawTriangleInterpolated(Vector3(p1.x, p1.y, p1.z), Vector3(p2.x, p2.y, p2.z), Vector3(p3.x, p3.y, p3.z), Color::RED, Color::BLUE, Color::GREEN, zBuf, &texture, uv0, uv1, uv2, zBuf_flag);
			}
			else {
				framebuffer->DrawTriangleInterpolated(Vector3(p1.x, p1.y, p1.z), Vector3(p2.x, p2.y, p2.z), Vector3(p3.x, p3.y, p3.z), Color::RED, Color::BLUE, Color::GREEN, zBuf, nullptr, uv0, uv1, uv2, zBuf_flag);
			}
			break;
		}
